"""Example for collect data functions with Airbot.

This example involves Airbot initialization and starting a state machine
...
class DataFileManager:
    ...
    def init_keys(self, observation, camera_keys) -> None:
        ...
        #打印low_dim_time_keys
        print(f"self.low_dim_time_keys is" , self.low_dim_time_keys)
    ...
    def add_raw_data(self, observation: dict, current_frame: int) -> None:
        ...
        for key in self.image_keys:
            ...
            if not isinstance(img, np.ndarray):
                print(f"add_raw_data [WARNING] Frame {current_frame} - {key} is not a numpy array. Got type: {type(img)}")
                continue  # 或 raise ValueError(...) 终止执行

            # 可选：形状检查，确保是三通道图像
            if img.ndim != 3 or img.shape[2] not in [3, 4]:
                print(f"add_raw_data [WARNING] Frame {current_frame} - {key} has unexpected shape: {img.shape}")
                continue  # 或 raise
    ...
    def convert_data(self) -> None:
        ...
        self.template_dict["id"] = generated_uuid
        arm_keys = [
            k
    ...
    def save_data(self) -> None:
        ...
        for cam_index in range(self.cam_num):
            ...
            for i, frame in enumerate(self.template_dict["data"][cam_key]):
                ...
                # 也可以加入类型断言
                assert isinstance(img, np.ndarray), f"[ERROR] cam{cam_index+1} frame {i} is not a numpy array! Got: {type(img)}"
        ...
        bson_path = str(self.task_path / f"{self.current_episode}" / "data.bson")
        from dataloop import DataLoopClient
        dataloop = DataLoopClient(endpoint='192.168.215.80', username='admin', password='123456')
        dataloop.samples.upload_sample(bson_path=bson_path,project_id=70,client_id="qimeng")
        self.current_episode += 1
...
class OpenCVCamera:
    ...
    def connect(self) -> None:
        ...
        print("find_rgb_video_device_by_path 打开成功")
        #第三步：设置摄像头参数（如果有提供）
        ...
        print(f"设置fps参数成功,为" , actual_fps)
        print(f"设置width参数成功,为" , actual_width)
        print(f"设置height参数成功,为" , actual_height)
        ...
        for i in range(10):
            ...
            raise OSError(f"摄像头 {self.camera_index} 已连接但连续读取 10 帧都失败，可能是带宽不足或摄像头状态异常。")
...
class AIRBOTPlay:
    ...
    def reset(self) -> None:
        ...
        Resets the robots to their initial state by setting up each robot's mode and position.
        ...
        for i in range(args.leader_number):
            ...
            if leader_robot[i].get_current_state() != "ONLINE_TRAJ":
                assert leader_robot[i].online_mode(), (
                    "Leader robot %d online idle mode failed" % i
                )
            time.sleep(wait_time)

        # Reset follower robots
        for i in range(args.follower_number):
            if follower_robot[i].get_current_state() != "SLAVE_MOVING":
                assert follower_robot[i].online_mode(), (
                    "Follower robot %d online idle mode failed" % i
                )
                assert follower_robot[i].slave_mode(
                    int(leader_robot[i].params["ros_domain_id"])
                ), ("Follower robot %d slave waiting mode failed" % i)
                time.sleep(wait_time)
                assert follower_robot[i].follow_start(), (
                    "Follower robot %d slave moving mode failed" % i
                )

        # Set leader robots' target positions
        for i in range(args.leader_number):
            if args.leader_arm_type[i] == "replay":
                continue
            ...
            if args.leader_end_effector[i] not in ["E2B", "none"]:
                assert leader_robot[i].set_target_end_accurate(
    ...
    def reset(self):
        ...
        for i, robot in enumerate(self.follower_robot):
            robot.connect()
        if self.config.is_reset_continue_follow == "True":
            print("reset_continue_follow")
            self.follower_start_all()
        else:
            #停止跟随（如果有的话）
            self.stop_followers()
            #移动机械臂到指定位置
    ...
    def follower_start(self, leader, follower, delay: float = 0.01):
        ...
        """
        #let the follower arm move to leader arm first
        follower.set_speed_profile(SpeedProfile.SLOW)
        follower.switch_mode(RobotMode.PLANNING_POS)
        follower.move_to_joint_pos(leader.get_joint_pos())
    ...
    def follower_start_all(self):
        #同时启动从机械臂的跟随模式
        for i in range(self.config.follower_number):
            if(self.follower_following_flags[i] != True):
    ...
    def get_low_dim_data(self) -> Dict[str, any]:
        ...
        for i in range(args.leader_number):
            ...
            action_eef_jq.append(leader_robot[i].get_eef_pos()) #获取夹爪的张开幅度（如果没有夹爪或者夹爪没准备好，返回None）
            pose = leader_robot[i].get_end_pose()  # [[x, y, z], [x, y, z, w]] #获取末端的空间位姿
    ...
    def capture_observation(self) -> Dict[str, any]:
        ...
        for name in self.cameras:
            obs_act_dict[f"observation.images.{name}"] = images[name] #将读取到的图像转存储到obs列表中（要在这里检查图像类型吗）
        return obs_act_dict
    ...
    def exit(self) -> None:
        ...
        try:
            ...
            for i in range(self.config.leader_number):
                self.leader_robot[i].switch_mode(RobotMode.PLANNING_POS)  # RobotMode.PLANNING_POS
                self.leader_robot[i].move_to_joint_pos([0, 0, 0, 0, 0, 0])
            for i in range(self.config.follower_number):
                self.follower_robot[i].switch_mode(RobotMode.PLANNING_POS)  # RobotMode.PLANNING_POS
...
class DataCollecter:
    ...
    def RobotInit(self) -> None:
        ...
        try:
            # Initial Robot
            self.robot = AIRBOTPlay(self.config)
    ...
    def Save(self) -> None:
        ...
        self.SaveOver()
        #save over kater continue follow
        if self.config.is_reset_continue_follow == "True":
            self.follower_start_all()
        self.change_flag = True
